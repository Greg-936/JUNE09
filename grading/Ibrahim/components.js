...
function Tooltip(text, child) {
  return el("div", { className: "tooltip-host" },
    child,
...
function ScoreRow(criterion, adminMode, onUpdate) {
  ...
  const miniBar = el("div", { className: "mini-bar-track" },
    el("div", { className: "mini-bar-fill", style: { width: `${pct}%`, background: info.color } })
  );

  return el("tr", { className: "score-row", "data-id": criterion.id },
...
function InstructorComments(comments) {
  ...
  comments.forEach(c => body.appendChild(
    el("li", { className: "comment-item" },
